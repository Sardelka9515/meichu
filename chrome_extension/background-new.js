...
async function detectAIContent(srcUrl, type) {
  const apiEndpoint = "http://localhost:5005/api/analyze/img"; // 替換為您的 API URL
          ...
          if (response) {
            if (response.success) {
              ...
              const artificial = Math.round(
                response.results[0]["output_sausagee"] * 100
              ); // 來自 API 的準確度
              const human = 1 - artificial;
  ...
  if (request.action === "sendImageToBackend") {
    ...
    fetch(base64Image)
      ...
      .then((blob) => {
        ...
        return fetch("http://localhost:5005/api/analyze/img", {
          method: "POST",
          body: formData,
        });
      })
